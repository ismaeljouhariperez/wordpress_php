...
class SponsoMetaBox
{
    ...
    const META_KEY = 'ninetyninetyone_sponso';
    const NONCE = '_ninetyninetyone_sponso_nonce';
    ...
    ?>
        ...
        <!-- checked() is similar to ($value === '1' ? 'checked' :  '') -->
        <input type='checkbox' value='1' name='<?= self::META_KEY ?>' <?php checked($value,'1') ?> >
        <label for='nineninetyonesponso'>Cet article est-il sponsorisé ?</label>
    <?php 
    }
    ...
    public static function save($post)
    {
        ...
        && wp_verify_nonce($_POST[self::NONCE], self::NONCE)) // Permet de vérifier l'origine des données et des formulaires
        {
